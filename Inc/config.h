...
*/
#define BAT_FILT_COEF           6554       // battery voltage filter coefficient in fixed-point. coef_fixedPoint = coef_floatingPoint * 2^16. In this case 655 = 0.01 * 2^16, -> 6554 = 0.1
#define BAT_CALIB_REAL_VOLTAGE  3802  //3970      // input voltage measured by multimeter (multiplied by 100). In this case 43.00 V * 100 = 4300
...
#define ELECTRIC_BRAKE_MAX    350       // (0, 500) Maximum electric brake to be applied when input torque request is 0 (pedal fully released).
...
// ############################ VARIANT_USART SETTINGS ############################
  ...
  #ifdef DUAL_INPUTS
    #define FLASH_WRITE_KEY      0x1112  // Flash memory writing key. Change this key to ignore the input calibrations from the flash memory and use the ones in config.h
...
// ########################### UART SETIINGS ############################
  ...
  #define SERIAL_NMAX_CMD_OPEN_MODE  9999              //if command_in->speed == -1234 -> set timeoutflg and go into OPEN_MODE (shut off motors)  
#endif
#if defined(FEEDBACK_SERIAL_USART2) || defined(CONTROL_SERIAL_USART2) || defined(DEBUG_SERIAL_USART2) || defined(SIDEBOARD_SERIAL_USART2)
  #ifndef USART2_BAUD
